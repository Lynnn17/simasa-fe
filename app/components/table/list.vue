<template>
  <div
    :class="[
      'bg-white dark:bg-slate-800 rounded-sm overflow-hidden',
      elevationClass,
    ]"
  >
    <!-- Header Bar -->
    <div
      v-if="showHeader"
      :class="[
        'flex flex-wrap items-center justify-between gap-3 px-5 py-1',
        headerClass,
      ]"
    >
      ...
      <div class="flex items-center gap-1">
        <template
          v-for="action in validActions(actionToolbars, {})"
          :key="action.key"
        >
          <UiIconButton
    ...
    <div :class="contentPadding">
      ...
      <ClientOnly>
        <div
          ...
          class="flex flex-col md:flex-row gap-4 bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm mb-5"
        >
        ...
        <template #fallback>
          <div
            class="h-20 bg-gray-50 dark:bg-slate-800/50 rounded-2xl animate-pulse mb-5"
          ></div>
        </template>
      </ClientOnly>
      ...
      >
        ...
        >
          <slot :name="`item.${header.key}`" v-bind="slotProps">
            {{
              header.formatter
                ? header.formatter(slotProps.value)
                : slotProps.value
            }}
          </slot>
        </template>
        ...
        <template v-if="actions.length > 0" #item.actions="{ item }">
          <div class="flex items-center justify-center gap-1.5">
            <template
              v-for="action in validActions(actions, item)"
              :key="action.key"
            >
              <UiIconButton
                :icon="action.icon || 'mdi-help'"
...
const numberInc = computed(() => {
  const number =
    parseInt(String(filterLocal.value.pageNumber - 1)) * itemsPerPage.value ||
    0;
  return number;
});

const addAction = computed(() => {
  return props.actionToolbars?.find(
    (a) => a.type === "default" && a.show?.({}) !== false,
  );
});
